...
$(document).ready(function(){
          ...
          this.postData('/dynoblock/generate/' + layout, data, function(data){
      			callback(data.html);
            ...
            if(callback){
  			...
  			editBlock: function(rid, bid, callback){
    			var $this = this;
    			var data = this.addPageState();
    			this.postData('/dynoblock/edit/' + rid + '/' + bid, data, function(data){
      			console.log(data);
      			callback(data.html);
      			if(data.commands.length > 0){
        			if(typeof(data.commands) == 'string'){
          ...
          this.blockAddElement(function(selector){
            _this.region.append(selector);
  			...
  			this.renderForm = function(dyno_form, layout_id){
    			this.removeAddBlock();
    			var bid = this.generateBlockId();
    			var form = $(this.blockForm(dyno_form, layout_id, bid));
    			this.region.append(form);
  			}
  			
  			this.blockForm = function(dyno_form, layout_id, bid, type){
    			type = !type ? 'save' : type;
    			...
    			form += '<input type="hidden" name="layout_id" value="'+ layout_id +'"/>';
  		...
  		$(document).on('click', '.dynoblock-edit', function(){
			...
			$(document).on('click', '.dynoblock-layout', function(){
  			var layout_id = $(this).data('dyno-lid');
  			var rid = $(this).closest('.dyno-add-block').data('dyno-rid');
  			var region = DynoBlocks.getRegion(rid);
  			if(region){
    			DynoBlocks.getLayout(layout_id, function(form){
      			if(form){
        			region.renderForm(form, layout_id);
      			}
    			});
  			}
  		});
